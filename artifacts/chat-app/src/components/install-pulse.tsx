...
declare global { interface Window { pulseDesktop?: { installed: boolean; platform: string; version: string }; Capacitor?: { isNativePlatform?: () => boolean } } }

function isNativeApp() { return Boolean(window.pulseDesktop?.installed || window.Capacitor?.isNativePlatform?.()); }
...
export function InstallPulse() {
  ...
  const deviceName = device === "ios" ? "iPhone / iPad" : device === "android" ? "Android" : device === "mac" ? "macOS" : device === "windows" ? "Windows" : device === "linux" ? "Linux" : "Computer";
  ...
  const href = DOWNLOADS[device] ?? RELEASES;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-[2rem] border border-white/15 bg-background/95 shadow-2xl p-6 sm:p-7">
        ...
        <p className="mt-2 text-sm text-muted-foreground">A real Pulse application with its own window, icon and launcher entry — not just a browser shortcut.</p>
        ...
        <div className="mt-5 rounded-2xl bg-primary/8 border border-primary/15 p-4 text-sm"><p className="font-semibold">Your device: {deviceName}</p><p className="text-muted-foreground mt-1">The button takes you straight to the native Pulse build when one is available.</p></div>
        <div className="flex gap-3 mt-6"><a href={href} className="flex-1"><Button className="w-full"><Download className="w-4 h-4 mr-2" />{label}</Button></a><Button variant="outline" onClick={() => setOpen(false)}>Not now</Button></div>
