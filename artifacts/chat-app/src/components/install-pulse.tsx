...
const DISMISS_KEY = "pulse-install-dismissed-session";
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/55 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-[2rem] border border-white/15 bg-background/95 shadow-2xl p-6 sm:p-7">
        ...
        {ios ? (
          <div className="mt-5 rounded-2xl bg-primary/8 border border-primary/15 p-4 text-sm">
            <p className="font-semibold">Install on iPhone or iPad</p>
            <p className="text-muted-foreground mt-1">Tap <b>Share</b> in Safari, then choose <b>Add to Home Screen</b>. Pulse will open from your Home Screen like an app.</p>
          </div>
        ) : (
          <p className="mt-5 text-xs text-muted-foreground">Your browser will show its normal install confirmation. If it doesn't support one-click installation, use the browser's <b>Install app</b> or <b>Add to Home screen</b> option.</p>
        )}

        <div className="flex gap-3 mt-6">
          {!ios && <Button onClick={install} className="flex-1"><Download className="w-4 h-4 mr-2" />Install Pulse</Button>}
          <Button variant="outline" onClick={() => setOpen(false)} className={ios ? "w-full" : "flex-1"}>Not now</Button>
        </div>
      </div>
    </div>
  );
