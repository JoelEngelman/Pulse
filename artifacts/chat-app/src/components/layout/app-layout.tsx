...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{if(!user)return;heartbeat.mutate(undefined);recordDailyPulse(user.username);recordNightActivity(user.username);const interval=setInterval(()=>{heartbeat.mutate(undefined);recordDailyPulse(user.username);recordNightActivity(user.username)},15000);return()=>clearInterval(interval)},[user?.username]);
  ...
  return <div className="flex h-[100dvh] w-full min-h-0 bg-background overflow-hidden text-foreground">
    ...
    <main className="flex-1 min-w-0 min-h-0 flex overflow-hidden bg-background relative">{children}</main>
