...
router.get("/communities", requireAuth, async (req, res) => {
  ...
  const counts = ids.length ? await db.select({ communityId: communityMembersTable.communityId, count: sql<number>`count(*)` })
    .from(communityMembersTable).where(sql`${communityMembersTable.communityId} in (${sql.join(ids.map((id) => sql`${id}`), sql`, `)})`)
    .groupBy(communityMembersTable.communityId) : [];
  const countMap = new Map(counts.map((c) => [c.communityId, Number(c.count)]));

  res.json(rows.map((r) => ({
    ...r.community,
    owner: { id: r.owner.id, username: r.owner.username, displayName: r.owner.displayName, avatarUrl: r.owner.avatarUrl },
    memberCount: countMap.get(r.community.id) ?? 0,
  })));
});
...
router.post("/communities", requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.status(409).json({ error: "That community name is already taken." });
  }
});

router.post("/communities/:id/join", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid community ID" });
  const [community] = await db.select().from(communitiesTable).where(eq(communitiesTable.id, id)).limit(1);
  if (!community) return res.status(404).json({ error: "Community not found" });
  await db.insert(communityMembersTable).values({ communityId: id, userId: req.session.userId! }).onConflictDoNothing();
...
router.delete("/communities/:id/join", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid community ID" });
  await db.delete(communityMembersTable).where(and(eq(communityMembersTable.communityId, id), eq(communityMembersTable.userId, req.session.userId!)));
...
router.get("/communities/:id/members", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid community ID" });
  const rows = await db.select({ user: usersTable, member: communityMembersTable })
    .from(communityMembersTable)
    .innerJoin(usersTable, eq(communityMembersTable.userId, usersTable.id))
    .where(eq(communityMembersTable.communityId, id))
    .orderBy(asc(communityMembersTable.joinedAt)).limit(100);
  res.json(rows.map((r) => ({ id: r.user.id, username: r.user.username, displayName: r.user.displayName, avatarUrl: r.user.avatarUrl, role: r.member.role })));
});
