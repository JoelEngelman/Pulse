...
router.post("/communities", requireAuth, async (req, res) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      const [community] = await tx.insert(communitiesTable).values({name,description,ownerId:req.session.userId!}).returning();
      await tx.insert(communityMembersTable).values({communityId:community.id,userId:req.session.userId!,role:"owner"});
      return community;
    });
    res.status(201).json({...result,memberCount:1,joined:true});
  } catch (error: any) {
    console.error("Failed to create community", error);
    if (error?.code === "23505") return res.status(409).json({error:"That community name is already taken."});
    res.status(500).json({error:"Couldn't create the community. Please try again."});
  }
});
