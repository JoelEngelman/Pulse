...
import { eq, and, lt, desc, ne, ilike, sql, inArray } from "drizzle-orm";
...
async function checkParticipant(convId: number, userId: number): Promise<boolean> {
  const [part] = await db.select().from(conversationParticipantsTable).where(and(eq(conversationParticipantsTable.conversationId, convId), eq(conversationParticipantsTable.userId, userId))).limit(1);
  return !!part;
}
...
router.get("/conversations/:conversationId/messages", requireAuth, async (req, res) => {
  ...
  if (isNaN(convId) || !(await checkParticipant(convId, me))) { res.status(404).json({ error: "Conversation not found" }); return; }

  const limit = Math.min(parseInt((req.query["limit"] as string) ?? "50"), 100);
  const before = req.query["before"] ? parseInt(req.query["before"] as string) : undefined;
  const condition = before ? and(eq(messagesTable.conversationId, convId), lt(messagesTable.id, before)) : eq(messagesTable.conversationId, convId);

  const rows = await db.select({ message: messagesTable, sender: usersTable }).from(messagesTable).leftJoin(usersTable, eq(messagesTable.senderId, usersTable.id)).where(condition).orderBy(desc(messagesTable.createdAt)).limit(limit);
  const ids = rows.map(r => r.message.id);
  const reactionRows = ids.length ? await db.select().from(messageReactionsTable).where(inArray(messageReactionsTable.messageId, ids)) : [];
  const reactionMap = new Map<number, Map<string, number[]>>();
  for (const row of reactionRows) {
    if (!reactionMap.has(row.messageId)) reactionMap.set(row.messageId, new Map());
    const byEmoji = reactionMap.get(row.messageId)!;
    if (!byEmoji.has(row.emoji)) byEmoji.set(row.emoji, []);
    byEmoji.get(row.emoji)!.push(row.userId);
  }
  const results = rows.map(({ message: msg, sender }) => ({
    id: msg.id,
    conversationId: msg.conversationId,
    senderId: msg.senderId,
    sender: sender ? toPublicUser(sender) : null,
    ...
    editedAt: msg.editedAt?.toISOString() ?? null,
    createdAt: msg.createdAt.toISOString(),
    reactions: Array.from(reactionMap.get(msg.id)?.entries() ?? []).map(([emoji, userIds]) => ({ emoji, count: userIds.length, userIds })),
  })).reverse();
  res.json(results);
});
...
router.patch("/conversations/:conversationId/messages/:messageId", requireAuth, async (req, res) => {
  ...
  if (isNaN(convId) || isNaN(msgId)) { res.status(400).json({ error: "Invalid IDs" }); return; }
  const parsed = EditMessageBody.safeParse(req.body); if (!parsed.success) { res.status(400).json({ error: "Invalid input" }); return; }
  ...
  if (!existing || existing.senderId !== me) { res.status(403).json({ error: "Forbidden" }); return; }
  await db.update(messagesTable).set({ content: parsed.data.content, editedAt: new Date() }).where(eq(messagesTable.id, msgId));
  const result = await buildMessage(msgId); res.json(result);
});
...
router.post("/conversations/:conversationId/messages/:messageId/reactions", requireAuth, async (req, res) => {
  const msgId = parseInt(String(req.params["messageId"] ?? "")); const me = req.session.userId!;
  const parsed = AddReactionBody.safeParse(req.body); if (!parsed.success) { res.status(400).json({ error: "Invalid input" }); return; }
  await db.execute(sql`INSERT INTO message_reactions (message_id, user_id, emoji) VALUES (${msgId}, ${me}, ${parsed.data.emoji}) ON CONFLICT (message_id, user_id, emoji) DO NOTHING`);
  const result = await buildMessage(msgId); res.json(result);
});
...
router.delete("/conversations/:conversationId/messages/:messageId/reactions/:emoji", requireAuth, async (req, res) => {
  ...
  await db.delete(messageReactionsTable).where(and(eq(messageReactionsTable.messageId, msgId), eq(messageReactionsTable.userId, me), eq(messageReactionsTable.emoji, emoji)));
  const result = await buildMessage(msgId); res.json(result);
});

router.get("/messages/search", requireAuth, async (req, res) => {
  const q = (req.query["q"] as string)?.trim(); const me = req.session.userId!;
  ...
  const results = await Promise.all((rows.rows as any[]).map(async (row) => ({ message: await buildMessage(row.message_id), conversationId: row.conversation_id })));
